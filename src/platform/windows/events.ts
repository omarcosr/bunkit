...
export function dispatch(): void {
  for (const e of drain()) {
    ...
    if (fn) {
      try {
        if (e.type === EVT_CONTROL_STATE) fn(e.value1, e.value2 !== 0, e);
        else if (e.type === EVT_TEXT_CHANGED || e.type === EVT_TEXT_SUBMIT || e.type === EVT_INPUT_KEY) fn(e.text, e);
